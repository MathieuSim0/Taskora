    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="card mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* Filtre Status */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">{t('tasks.filters.status')}</label>
            <select
              value={statusFilter}
          ...
          {/* Filtre Priority */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">{t('tasks.filters.priority')}</label>
            <select
              value={priorityFilter}
              onChange={(e) => setPriorityFilter(e.target.value)}
              className="input"
            >
              <option value="ALL">{t('tasks.filters.allPriorities')}</option>
          ...
          {/* Filtre Project */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">{t('tasks.filters.project')}</label>
            <select
              value={projectFilter}
      ...
      {filteredTasks.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl shadow-md border border-slate-100">
          ...
          <h3 className="text-xl font-semibold text-slate-900 mb-2">
            {tasks.length === 0 ? t('tasks.empty.noTasks') : t('tasks.empty.noMatch')}
          </h3>
        ...
        <div className="p-6 border-b border-slate-200 sticky top-0 bg-white z-10">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-slate-900">
              {task ? t('tasks.edit.title') : t('tasks.create.title')}
            </h2>
            <button
              onClick={onClose}
              type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {task && (
            <div className="border-t border-slate-200 pt-6 mt-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">{t('tasks.form.attachments')}</h3>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {task ? t('tasks.edit.update') : t('tasks.create.create')}
                </>
              )}
            </button>
          </div>
